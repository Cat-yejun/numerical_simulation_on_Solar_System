...
def rungekutta_method(particle, t0, t1, h):
    ...
    return np.array(ts), np.array(xs), np.array(ys), np.array(zs), np.array(vxs), np.array(vys), np.array(vzs)
...
if __name__ == '__main__':
    ...
    T, realX, realY, realZ, VX, VY, VZ = rungekutta_method(particle, initialTime, initialTime + 2, interval)
